...
- `--color-page-bg`
- `--color-hero-well` / `--color-hero-well-rgb` — recessed hero background (darker than page)
...
- `--shadow-panel` — used by `.section-panel`

## Home layout surfaces

- `.home-hero-well` — recessed hero band (darker surface, inset shadow)
- `.hero-tile-canvas` — canvas tile grid behind copy (`HeroTileField`)
- `.section-band`, `.section-band--featured` — full-bleed bands (featured work section)
- `.section-panel` — inset bordered panels for secondary sections
- `.section-atmosphere` — organic blobs + structural grid backdrops (`SectionAtmosphere`)
- `.home-section-title`, `.home-section-desc`, `.home-section-lead`, `.home-section-body` — unified type below hero
- `.home-principles`, `.home-credential-list` — structured mini-blocks in panel sections
- `.hero-intro` — greeting line above the display headline

Hero tile tokens: `--hero-tile-gap`, `--hero-tile-flip-duration`
...
- `components/home/HeroTileField.tsx` — hero tile grid uses RGB surface/accent tokens via CSS
